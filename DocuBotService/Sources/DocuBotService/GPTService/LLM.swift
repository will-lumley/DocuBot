...
open class LLM: ObservableObject, @unchecked Sendable {
    ...
    public init(
        ...
        print("TopK: \(topK)")
        print("TopP: \(topP)")
        print("StopSequence: \(stopSequence ?? "nil")")
        print("Temperature: \(temp)")
        print("MaxTokenCount: \(maxTokenCount)")
    ...
    private func predictNextToken() async -> Token {
        guard shouldContinuePredicting else {
    ...
    private func process(_ token: Token, to output: borrowing AsyncStream<String>.Continuation) -> Bool {
        struct saved {
            nonisolated(unsafe) static var stopSequenceEndIndex = 0
            nonisolated(unsafe) static var letters: [CChar] = []
        }
...
public struct Template {
    ...
    nonisolated(unsafe) public static let mistral = Template(
        user: ("[INST] ", " [/INST]"),
        bot: ("", "</s> "),
        stopSequence: "</s>",
        systemPrompt: nil
